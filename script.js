const apikey = 'c519bd8b-9994-4215-9739-6bbd94a65775';
const apihost = 'https://todo-api.coderslab.pl';


function apiCreateOperationForTask(taskId, description) { //funkcja przyjmuje dwa parametry,identyfikator zadania i opis operacji.
    return fetch( //wywołanie metody fetch, zwraca obietnicę "Promise" która zostanie rozwiązana gdy odpowiedź z serwera zostanie otrzymana.
        apihost + '/api/tasks/' + taskId + '/operations', //funkcja wykonuje zapytanie do API, używa metody fetch, tworzy nową operację dla danego zadania. Zapytanie kieruje do adresu api/itd
        {
            headers: { Authorization: apikey, 'Content-Type': 'application/json' }, //apikey czyli wartość klucza API. JSON typ danych wysyłanych do serwera.
            body: JSON.stringify({ description: description, timeSpent: 0 }), // JSON zawiera pola opis operacji i czas poświęcony operacji.
            method: 'POST' // metoda którą wysyłane jest zapytanie na serwer.
        }
    ).then(
        function (resp) {
            if(!resp.ok) {
...
//{"error":false,"data":{"apiKey":"c519bd8b-9994-4215-9739-6bbd94a65775"}} TODOAPIKEY